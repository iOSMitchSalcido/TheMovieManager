...
class MovieDetailViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let movie = movie {
            ...
            print("Average Vote: \(movie.voteAverage!)")
            
            // set the title
            var titleString = movie.title
            
            if let releaseYear = movie.releaseYear {
                titleString = titleString + "(\(releaseYear))"
            ...
            if let rating = movie.voteAverage {
                titleString = titleString + " - \(rating)"
            }
        ...
        TMDBClient.sharedInstance().postToFavorites(movie!, favorite: shouldFavorite) { (statusCode, error) in
            if let error = error {
                ...
                if statusCode == 1 || statusCode == 12 || statusCode == 13 {
                    ...
                    print("Unexpected status code \(String(describing: statusCode))")
                }
            }
        }
    }
