...
extension TMDBClient {
    ...
    */
    ...
    func getWatchlistMovies(_ completionHandlerForWatchlist: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        ...
        let parameters = [TMDBClient.ParameterKeys.SessionID: TMDBClient.sharedInstance().sessionID]
        var method = Methods.AccountIDWatchlistMovies
        method = substituteKeyInMethod(method, key: TMDBClient.URLKeys.UserID, value: String(TMDBClient.sharedInstance().userID!))!
        print(method)
        /* 2. Make the request */
        let _ = taskForGETMethod(method, parameters: parameters as [String : AnyObject]) {
            (results, error) in
            ...
            if let error = error {
                ...
                if let results = results?[TMDBClient.JSONResponseKeys.MovieResults] as? [[String:AnyObject]] {
                    ...
                    completionHandlerForWatchlist(nil, NSError(domain: "getWatchlistMovies", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse getWatchlistMovies"]))
                }
            }
        }
    }
        ...
        let _ = taskForPOSTMethod(mutableMethod, parameters: parameters as [String : AnyObject], jsonBody: jsonBody) {
            ...
            if let error = error {
                ...
                if let results = results?[TMDBClient.JSONResponseKeys.StatusCode] as? Int {
                    ...
                    completionHandlerForWatchlist(nil, NSError(domain: "postToWatchlist", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse postToWatchlist"]))
                }
            }
        }
    }
